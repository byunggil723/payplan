...
class DetailActivity : AppCompatActivity() {
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        if (memo != null) {
                            ...
                            startCountdown(memo, selectedDate) // 메모 로드 후 카운트다운 시작
    ...
    private fun displayMemoDetails(memo: DataModel) {
        ...
        binding.memoDate.text = "${memo.startYear}-${memo.startMonth}-${memo.startDate} ~ ${memo.endYear}-${memo.endMonth}-${memo.endDate}"
    ...
    private fun startCountdown(memo: DataModel, selectedDate : String) {
        runnable = object : Runnable {
            override fun run() {
                val currentTime = Calendar.getInstance(TimeZone.getTimeZone("Asia/Seoul")).timeInMillis
                Log.d("currentTime", "${currentTime}")
                val endTime = getEndTimeInMillis(memo, selectedDate)
                val startTime = getStartTimeInMillis(memo, selectedDate)
                ...
                if (endTime > currentTime && currentTime > startTime) {
                    ...
                    binding.remainingTime.text = "시작 전"
                }
                else if (currentTime > endTime) {
                    // 시간이 지났거나 종료 시간이 현재 시간과 정확히 일치할 경우 처리
    ...
    // 메모의 시작 시간을 밀리초로 변환하는 함수
    private fun getStartTimeInMillis(memo: DataModel, selectedDate : String): Long {
        ...
        // 종료 시간 설정
        calendar.set(year, month - 1, date, memo.startHour, memo.startMinute, 0)
        Log.d("dataCheck", "${year}-${month}-${date}-${memo.startHour}-${memo.startMinute}")

        return calendar.timeInMillis
    }

    // 메모의 종료 시간을 밀리초로 변환하는 함수
    private fun getEndTimeInMillis(memo: DataModel, selectedDate : String): Long {
        val calendar = Calendar.getInstance(TimeZone.getTimeZone("Asia/Seoul"))
        val parts = selectedDate.split("-")
        val year = parts[0].toInt()
        val month = parts[1].toInt()
        ...
        // 종료 시간 설정
        calendar.set(year, month - 1, date, memo.endHour, memo.endMinute, 0)
        Log.d("dataCheck", "${year}-${month}-${date}-${memo.endHour}-${memo.endMinute}")
